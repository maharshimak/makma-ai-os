...
function addMessage(role, content, error = false) {
  ...
  const meta = role === 'user' ? 'You' : role === 'assistant' ? "Mak'ma" : 'Runtime';
  article.innerHTML =
...
function renderPlan(plan) {
  if (!plan || !plan.length) {
    els.plan.innerHTML = '<div class="empty-state"><span>NO ACTIVE PLAN</span><p>No planning steps were emitted.</p></div>';
    return;
  }

  els.plan.innerHTML = '<div class="trace-list">' + plan.map((step, index) => {
    const args = step.arguments && Object.keys(step.arguments).length
      ? '<code class="trace-code">' + escapeHtml(JSON.stringify(step.arguments, null, 2)) + '</code>'
      : '';
    return (
      '<article class="trace-step">' +
        '<div class="trace-step-head">' +
          '<div><span class="trace-index">' + String(index + 1).padStart(2, '0') + '</span>' +
          '<div class="trace-title">' + escapeHtml(step.kind === 'tool' ? step.tool_name : 'respond') + '</div></div>' +
...
function renderTools(results) {
  if (!results || !results.length) {
    els.tools.innerHTML = '<div class="empty-state"><span>NO TOOL EXECUTION</span><p>This run completed without a tool call.</p></div>';
    return;
  }

  els.tools.innerHTML = '<div class="tool-list">' + results.map((result) => (
    '<article class="tool-result">' +
      '<div class="tool-result-head">' +
        '<div class="tool-name">' + escapeHtml(result.tool_name) + '</div>' +
        '<span class="badge ' + (result.ok ? 'badge-success' : 'badge-failure') + '">' +
          (result.ok ? 'success' : 'failed') +
        '</span>' +
      '</div>' +
      '<p>' + escapeHtml(result.ok ? result.output : (result.error || 'Tool failed')) + '</p>' +
      '<code class="trace-code">latency_ms: ' + escapeHtml(formatNumber(result.latency_ms || 0, 3)) + '</code>' +
    '</article>'
  )).join('') + '</div>';
}
...
function renderMemory(matches) {
  if (!matches || !matches.length) {
    els.memory.innerHTML = '<div class="empty-state"><span>NO MATCHES</span><p>No relevant session memories were found for this query.</p></div>';
    return;
  }

  els.memory.innerHTML = '<div class="memory-list">' + matches.map((match) => (
    '<article class="memory-match">' +
      '<div class="memory-match-head">' +
        '<div class="memory-role">' + escapeHtml(match.role) + '</div>' +
        '<span class="memory-score">score ' + escapeHtml(formatNumber(match.score || 0, 3)) + '</span>' +
      '</div>' +
      '<p>' + escapeHtml(match.content) + '</p>' +
      (match.created_at ? '<code class="trace-code">' + escapeHtml(match.created_at) + '</code>' : '') +
    '</article>'
  )).join('') + '</div>';
}
...
function renderTelemetry(summary) {
  ...
  const successRate = count ? ((count - failures) / count) * 100 : 100;

  els.telemetry.innerHTML =
    '<div class="telemetry-stat"><span>Runtime P50</span><strong>' + escapeHtml(formatNumber(summary.p50_latency_ms || 0, 2)) + ' ms</strong></div>' +
    '<div class="telemetry-stat"><span>Runtime P95</span><strong>' + escapeHtml(formatNumber(summary.p95_latency_ms || 0, 2)) + ' ms</strong></div>' +
    '<div class="telemetry-stat"><span>Success rate</span><strong>' + escapeHtml(formatNumber(successRate, 1)) + '%</strong></div>' +
    '<div class="telemetry-stat"><span>Runs observed</span><strong>' + escapeHtml(count) + '</strong></div>';
}
...
function demoPlan(message) {
  ...
  const mathPattern = /\b(?:calculate|compute|evaluate)\s+([0-9\s+\-*/().%]+)/gi;
  ...
  if (!candidates.length && /^[0-9\s+\-*/().%]+$/.test(text) && /\d/.test(text)) {
    candidates.push({
      position: 0,
      step: {
        id: newRunId(),
  ...
  const memoryMatch = /\b(?:search|check|look in|find in)\s+(?:my\s+)?memory(?:\s+for)?\s*(.*)/i.exec(text);
  if (memoryMatch) {
    const raw = (memoryMatch[1] || '').trim();
    const query = raw.split(/\s+and\s+(?=(?:calculate|compute|evaluate)\b)/i, 1)[0].trim() || text;
    candidates.push({
...
async function runDemo(message) {
  ...
  for (const step of plan) {
    ...
    if (step.tool_name === 'calculator') {
      try {
        ...
        const rendered = Number.isInteger(value) ? String(value) : String(Number(value.toFixed(10)));
        results.push({
          tool_name: 'calculator',
          ok: true,
    ...
    if (step.tool_name === 'memory_search') {
      ...
      results.push({
        ...
        output: matches.length
          ? matches.map((item) => '[score=' + formatNumber(item.score, 3) + '] ' + item.role + ': ' + item.content).join(' | ')
  ...
  if (successful.length) {
    response = "Mak'ma completed the requested tool work: " + successful.map((result) => result.output).join(' | ');
  } else if (results.length) {
    response = "Mak'ma could not complete the requested tool work safely.";
  } else {
    response = "Browser Demo Mode recorded this message in session memory. Connect a Mak'ma API to run the real backend provider.";
  }

  state.memory.push({ role: 'assistant', content: response, created_at: new Date().toISOString() });
  ...
  const result = {
    ...
    metrics: {
      provider_latency_ms: Math.max(0.1, latency * 0.2),
      tool_latency_ms: results.reduce((sum, item) => sum + Number(item.latency_ms || 0), 0),
...
function demoTelemetrySummary() {
  const latencies = state.telemetry.map((event) => event.latency_ms).sort((a, b) => a - b);
...
async function refreshTelemetry() {
  if (state.mode === 'connected') {
    try {
      const response = await fetch(state.apiBase + '/v1/telemetry?operation=runtime.run');
      if (!response.ok) throw new Error();
      renderTelemetry(await response.json());
      return;
    } catch {
      renderTelemetry({ count: 0, failures: 0, p50_latency_ms: 0, p95_latency_ms: 0 });
      return;
    }
  }
...
async function recall(query) {
  ...
  if (state.mode === 'connected') {
    const url = state.apiBase + '/v1/sessions/' + encodeURIComponent(state.sessionId) +
      '/recall?q=' + encodeURIComponent(normalized);
    const response = await fetch(url);
    if (!response.ok) throw new Error('Recall returned HTTP ' + response.status + '.');
...
function updateRun(result) {
  ...
  els.toolCount.textContent = String(result.metrics?.tool_calls ?? result.tool_results?.length ?? 0);
  renderPlan(result.plan || []);
  renderTools(result.tool_results || []);
  refreshTelemetry();

  const memoryTool = (result.tool_results || []).find((item) => item.tool_name === 'memory_search');
  if (memoryTool && memoryTool.ok) {
    const plannedMemory = (result.plan || []).find((step) => step.tool_name === 'memory_search');
...
async function submitMessage(message) {
  ...
  try {
    const result = state.mode === 'connected'
      ? await runConnected(normalized)
      : await runDemo(normalized);
    updateRun(result);
    addMessage('assistant', result.response || 'Runtime completed without a text response.');
  } catch (error) {
    const messageText = error instanceof Error ? error.message : 'Runtime request failed.';
...
function applyMode() {
  ...
  els.modeLabel.textContent = connected ? 'Connected Runtime' : 'Browser Demo Mode';
...
async function verifyConnection() {
  ...
  try {
    ...
    if (health.status !== 'ok') throw new Error('Runtime health response is not OK.');
    ...
    els.feedback.textContent = 'Connected. Real runtime requests are now enabled.';
    applyMode();
    setTimeout(() => els.dialog.close(), 650);
  } catch (error) {
    els.feedback.className = 'connection-feedback error';
    els.feedback.textContent = error instanceof Error
...
function resetSession() {
  ...
  els.plan.innerHTML = '<div class="empty-state"><span>NO ACTIVE PLAN</span><p>Run a request to inspect ordered planning steps.</p></div>';
  els.tools.innerHTML = '<div class="empty-state"><span>NO TOOL EXECUTION</span><p>Tool results, status and latency will appear here.</p></div>';
  els.memory.innerHTML = '<div class="empty-state"><span>SESSION MEMORY</span><p>Search the current session to inspect ranked memory matches.</p></div>';
  renderTelemetry(demoTelemetrySummary());
  clearConversation();
}
...
els.memorySearch.addEventListener('click', () => {
  recall(els.memoryQuery.value).catch((error) => {
    renderMemory([]);
    addMessage('system', error instanceof Error ? error.message : 'Memory recall failed.', true);
  });
});
...
els.connect.addEventListener('click', () => {
  ...
  els.feedback.className = 'connection-feedback';
  els.feedback.textContent = state.mode === 'connected'
