...
(() => {
    ...
    const parseNumberList = (value, label, { min = 0 } = {}) => {
        ...
        if (values.some((v) => v < min))
            throw new Error(label + " values must be at least " + min + ".");
        return values;
    };
    ...
    const table = (rows) => {
        if (!rows?.length)
            return '<div class="empty-note">No rows returned.</div>';
    ...
    const trace = (items) =>
        setHTML(
            ...
            items
                .map(
                    (x, i) =>
                        '<div class="step"><div class="kicker">' +
    ...
    const error = (message) => {
        ...
        return setHTML(
            "#result",
            '<div class="error-box"><strong>Could not run this workflow</strong><p>' +
                esc(message) +
                "</p></div>",
        );
    };
    const shell = (inner) => {
        ...
        $$(".field", root).forEach((field, index) => {
            const label = $("label", field),
                control = $("input, textarea, select", field);
    ...
    const psi = (expected, actual, epsilon = 1e-6) => {
        if (expected.length !== actual.length || !expected.length)
            throw new Error("Distributions must use equal non-empty bins.");
        if ([...expected, ...actual].some((v) => !Number.isFinite(v) || v < 0))
            throw new Error(
                "Distribution bins must be finite and non-negative.",
            );
        const et = expected.reduce((a, b) => a + b, 0),
            at = actual.reduce((a, b) => a + b, 0);
        if (et <= 0 || at <= 0)
