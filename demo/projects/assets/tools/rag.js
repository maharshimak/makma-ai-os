'use strict';
(() => {
  const M=window.MAKMA,{ $,esc,fmt,tokens,uniq,mean,readNumber,metrics,badge,setHTML,trace,error,shell,finish,copyText}=M;
  ...
  async function tokenDigest(token){
    if(!globalThis.crypto?.subtle) throw new Error('This browser does not support Web Crypto required for deterministic semantic retrieval.');
    if(!digestCache.has(token)){
      digestCache.set(token,crypto.subtle.digest('SHA-256',new TextEncoder().encode(token)).then(x=>new Uint8Array(x)));
    }
    return digestCache.get(token);
  }
  async function vector(text,dims=256){
    const v=Array(dims).fill(0),ts=String(text).toLowerCase().split(/\s+/).filter(Boolean);
    for(const token of ts){
      const bytes=await tokenDigest(token);
      let bucket=0n;
      for(let i=0;i<8;i++) bucket=(bucket<<8n)|BigInt(bytes[i]);
      const idx=Number(bucket%BigInt(dims)),sign=(bytes[0]&1)?-1:1;
      v[idx]+=sign;
    }
  ...
  async function semantic(docs,q,topK=20){
    const qv=await vector(q),rows=[];
    for(const d of docs){const sc=cosine(qv,await vector(d.text));rows.push({doc:d,score:sc,lexical:0,semantic:sc});}
  ...
  function retrievalMetrics(ranked,relevant){
    ...
    return {recall:relevant.size?hits.length/relevant.size:1,precision:retrieved.length?hits.length/retrieved.length:0,mrr:first<0?0:1/(first+1),hit:hits.length?1:0,hits};
  }
  async function run(){
    const started=performance.now();
    try{
      const docs=parseCorpus($('#corpus').value),q=$('#query').value.trim(),topK=readNumber('#topk','Top K',{min:1,max:10,integer:true}),candidateK=readNumber('#candidatek','Candidate K',{min:1,max:50,integer:true}),rrfK=readNumber('#rrfk','RRF k',{min:1,max:1000}),k1=readNumber('#k1','BM25 k1',{min:.1,max:5}),b=readNumber('#b','BM25 b',{min:0,max:1});
      ...
      for(const p of planned) rankings.push(rrf([bm25(docs,p,candidateK,k1,b),await semantic(docs,p,candidateK)],rrfK));
      ...
      setHTML('#result',metrics([['Documents',docs.length],['Planned queries',planned.length],['Candidates',fused.length],['Returned',ranked.length]])+
        ...
        '<div class="section-title">Retrieval evaluation @ '+topK+'</div>'+metrics([['Recall@K',relevant.size?(ev.recall*100).toFixed(1)+'%':'N/A',relevant.size?(ev.recall===1?'good':'warn'):''],['Precision@K',(ev.precision*100).toFixed(1)+'%'],['MRR',fmt(ev.mrr,3),ev.mrr?'good':'bad'],['Hit Rate@K',ev.hit?'100%':'0%',ev.hit?'good':'bad']])+
  ...
  if(scenario==='invalid'){$('#topk').value='5';$('#candidatek').value='2';}
  run();
})();
