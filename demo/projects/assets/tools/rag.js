...
(() => {
  ...
  function run(){
    ...
    try{
      const docs=parseCorpus($('#corpus').value),q=$('#query').value.trim(),topK=clamp(parseInt($('#topk').value||'5',10),1,10);
      if(!docs.length||!q)throw new Error('Add at least one document and a query.');
      const planned=plan(q),rankings=[];
      planned.forEach(p=>rankings.push(rrf([bm25(docs,p),semantic(docs,p)])));
      const fused=rrf(rankings),ranked=rerank(q,fused,topK),context=ranked.map(r=>'['+r.doc.id+'] '+r.doc.text).join('\n\n');
      setHTML('#result',metrics([['Documents',docs.length],['Planned queries',planned.length],['Candidates',fused.length],['Returned',ranked.length]])+
        ...
        ranked.map((r,i)=>'<div class="card"><div class="row split"><div>'+badge('#'+(i+1))+' '+badge(r.doc.id,'ok')+'</div><strong>'+fmt(r.score,4)+'</strong></div><p>'+esc(r.doc.text)+'</p><div class="score-line"><span>BM25 '+fmt(r.lexical,3)+'</span><span>semantic '+fmt(r.semantic,3)+'</span><span>overlap '+(r.overlap*100).toFixed(0)+'%</span></div></div>').join('')+
      ...
      trace([{label:'planner',title:'Multi-query planning',text:planned.join(' · ')},{label:'retrieval',title:'Hybrid retrieval',text:'BM25 lexical ranking + deterministic hashed-vector semantic ranking run for every planned query.'},{label:'fusion',title:'Reciprocal-rank fusion',text:'Fused '+fused.length+' unique candidates with RRF (k=60).'},{label:'rerank',title:'Transparent reranking',text:'Overlap-aware reranking returned top '+ranked.length+'.'}]);
    }catch(e){error(e.message);}
    finish(started);
  }
  shell('<div class="field"><label>Corpus · one document per line as <code>id | text</code></label><textarea id="corpus" class="tall">memory | SQLite stores persistent conversation memory for Mak\'ma sessions.\nretrieval | Hybrid retrieval combines BM25 lexical ranking with semantic vector ranking.\nfusion | Reciprocal-rank fusion combines independent rankings before reranking.\nevaluation | Retrieval quality can be measured with Recall@K, Precision@K, MRR and Hit Rate.\nadapters | OpenAI-compatible embedding and chat adapters can connect local or cloud model servers.</textarea></div><div class="field"><label>Query</label><textarea id="query">How does hybrid retrieval combine lexical and semantic search?</textarea></div><div class="field"><label>Top K</label><input id="topk" type="number" min="1" max="10" value="4"></div><button id="run" class="btn primary">Run real hybrid retrieval</button>');
  $('#run').onclick=run;run();
})();